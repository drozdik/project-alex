...
class Screen:
    ...
    def __init__(self, game_state, queue: Queue):
        # state
    ...
    def on_hero_precision_strike_btn_clicked(self):
        self.queue.put("precision_strike")

    def on_hero_aoe_strike_btn_clicked(self):
        self.queue.put("aoe_strike")

    def on_hero_combo_strike_btn_clicked(self):
        self.queue.put("combo_strike")

    def on_hero_block_btn_clicked(self):
    ...
    def draw_images_and_buttons(self):
        ...
        hero_attack_button = Button(self.frame, text="Attack and finish turn", command=self.on_hero_attack_btn_clicked)
        hero_attack_button.grid(row=1, column=0, columnspan=1)

        self.hero_heal_button = Button(self.frame, text="Heal", command=self.on_hero_heal_btn_clicked)
        self.hero_heal_button.grid(row=2, column=0, columnspan=1)

        hero_precision_strike_button = Button(self.frame, text="Precision Strike",
                                              command=self.on_hero_precision_strike_btn_clicked)
        hero_precision_strike_button.grid(row=3, column=0, columnspan=1)

        hero_aoe_strike_button = Button(self.frame, text="AOE Strike", command=self.on_hero_aoe_strike_btn_clicked)
        hero_aoe_strike_button.grid(row=4, column=0, columnspan=1)

        hero_combo_strike_button = Button(self.frame, text="Combo Strike", command=self.on_hero_combo_strike_btn_clicked)
        hero_combo_strike_button.grid(row=5, column=0, columnspan=1)

        hero_block_button = Button(self.frame, text="Block", command=self.on_hero_block_btn_clicked)
    ...
    def disable_healing_button(self):
    ...
    def show_number_of_health_potions_changed(self, potion_diff):
        self.update_hero_text()

    def show_hero_armor_changed(self, armor_diff):
        self.hero_status.config(text=f"Armor {'%+d' % armor_diff}", fg='blue')
        self.update_hero_text()
        time.sleep(0.5)
        self.hero_status.config(text="", fg='red')

    def show_hero_blocked(self):
        self.hero_status.config(text=f"Blocked")
        time.sleep(0.5)
        self.hero_status.config(text=f"")

    def show_monster_blocked(self, monster):
        status_label = self.get_monster_status_label(monster)
        status_label.config(text=f"Blocked")
        time.sleep(0.5)
        status_label.config(text=f"")

    def show_new_monsters_pack(self):
        print('updating stuff')
        self.update_monster1_image()
